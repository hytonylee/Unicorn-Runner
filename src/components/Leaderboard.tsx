...
export default function Leaderboard({
  ...
  return (
    <PanelCard id="leaderboard-panel">
      ...
      <PanelHeader
        ...
        title="PM Hall of Fame"
        action={isOffline && scores.length > 0 ? (
          <button
      ...
      <div className="space-y-2 max-h-64 overflow-y-auto pr-1 relative z-10" id="scores-container-list">
        {scores.length === 0 ? (
          ...
          scores
            ...
            .slice(0, 50)
            .map((item, idx) => {
              const obstacle = OBSTACLES_DATA[item.killedByAcronym] ?? {
                name: item.killedByAcronym || 'Unknown',
                emoji: '⚠️',
                fullName: item.killedByAcronym || 'Unknown',
              };
              const rankColor =
                  ...
                  <div className="flex items-center gap-3 text-left">
                    ...
                    <div>
                      <div className="text-xs font-bold font-sans text-slate-200 flex items-center gap-1.5 flex-wrap">
                        {item.name}
                        <Badge color="indigo">Lvl {item.level}</Badge>
                      </div>
                      ...
                      {item.killedByAcronym && (
                        <div className="text-[10px] text-slate-500 font-mono mt-0.5 flex items-center gap-1">
                          ...
                          <Badge color="purple" className="font-bold">{obstacle.name}</Badge>
                        </div>
                      )}
                    </div>
                  </div>
